...
public class RespValAddService {
    ...
    public void processAsync(String xml, String pathTxnId) {
        try { processFromNpci(xml, pathTxnId); } catch (Exception e) { System.err.println("RespValAddService (NPCI) ERROR: " + e.getMessage()); }
    }
    ...
    public void processFromSwitch(byte[] isoBytes, String pathTxnId) {
        ...
        try {
            if (txnIdForNpci != null && !txnIdForNpci.isBlank()) npciMockClient.sendRespValAdd(xml, txnIdForNpci);
            else npciMockClient.sendRespValAdd(xml);
