...
public class SwitchReqPayService {
    ...
    public void process(byte[] isoBytes, String pathTxnId) {
        String txnId = (pathTxnId != null && !pathTxnId.isBlank()) ? pathTxnId : "SWITCH_REQ_" + System.currentTimeMillis();
        ...
        // 4. Send XML to NPCI Mock Client (dynamic URL /imps/reqpay/{txnId})
        try {
            String response = (txnId != null && !txnId.isBlank()) ? npciMockClient.sendReqPay(xml, txnId) : npciMockClient.sendReqPay(xml);
            if (response != null) {
