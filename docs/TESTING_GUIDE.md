...
Configuration is in **application.yml**. Edit paths if your project location differs from `E:/Hitachi_Project`.
...
**HTTPS (SSL profile)** – add `-Dspring-boot.run.profiles=ssl` for each app:
...
```bash
# Linux/Mac – run with ssl profile
...
```powershell
# Windows PowerShell – run with ssl profile
cd Imps-backend; mvn spring-boot:run -Dspring-boot.run.profiles=ssl
...
Ensure keystore paths in application.yml are correct (default: `file:E:/Hitachi_Project/certs/...`).
...
For HTTPS on IMPS (port **8443**), run:
