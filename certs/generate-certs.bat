...
keytool -genkeypair -alias imps -keyalg RSA -keysize 2048 -validity 365 -keystore imps-keystore.jks -storepass IMPS-Backend -keypass IMPS-Backend -dname "CN=IMPS, OU=Dev, O=Hitachi, L=Mumbai, ST=MH, C=IN"

echo Creating Switch keystore...
keytool -genkeypair -alias switch -keyalg RSA -keysize 2048 -validity 365 -keystore switch-keystore.jks -storepass IMPS-Backend -keypass IMPS-Backend -dname "CN=Switch, OU=Dev, O=Hitachi, L=Mumbai, ST=MH, C=IN"

echo Creating NPCI keystore...
keytool -genkeypair -alias npci -keyalg RSA -keysize 2048 -validity 365 -keystore npci-keystore.jks -storepass IMPS-Backend -keypass IMPS-Backend -dname "CN=NPCI, OU=Dev, O=Hitachi, L=Mumbai, ST=MH, C=IN"
